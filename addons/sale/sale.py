...
class res_company(osv.Model):
    ...
    _columns = {
        'sale_note': fields.text('Default Terms and Conditions', translate=True,
...
class sale_order(osv.osv):
    ...
    _columns = {
        ...
        'order_line': fields.one2many('sale.order.line', 'order_id', 'Order Lines', readonly=True,
                                      states={'draft': [('readonly', False)], 'sent': [('readonly', False)]},
                                      copy=True),
        'invoice_ids': fields.many2many('account.invoice', 'sale_order_invoice_rel', 'order_id', 'invoice_id',
        ...
        for s in sale_orders:
            if s['state'] in ['draft', 'cancel']:
                ...
                raise osv.except_osv(_('Invalid Action!'),
                                     _('In order to delete a confirmed sales order, you must cancel it before!'))
        ...
        for val in invoices.values():
            if grouped:
                ...
                for order, il in val:
                    ...
                    cr.execute('insert into sale_order_invoice_rel (order_id,invoice_id) values (%s,%s)',
                               (order.id, res))
    ...
    def action_done(self, cr, uid, ids, context=None):
        for order in self.browse(cr, uid, ids, context=context):
            self.pool.get('sale.order.line').write(cr, uid, [line.id for line in order.order_line], {'state': 'done'},
                                                   context=context)
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.order_line:
                ...
                if line.procurement_ids:
                    ...
                    procurement_obj.check(cr, uid,
                                          [x.id for x in line.procurement_ids if x.state not in ['cancel', 'done']])
                    line.refresh()
                    ...
                    vals = self._prepare_order_line_procurement(cr, uid, order, line,
                                                                group_id=order.procurement_group_id.id, context=context)
        ...
        for line in order_lines:
            ...
            # link all  (6, 0, IDS)
                ...
                for line_id in line_ids:
                    ...
                    if prod and prod.taxes_id:
                        order_line.append(
                            [1, line_id, {'tax_id': [[6, 0, fiscal_obj.map_tax(cr, uid, fpos, prod.taxes_id)]]}])
                    else:
...
class sale_order_line(osv.osv):
    ...
    def _base_amount_line(self, cr, uid, ids, field_name, arg, context=None):
        tax_obj = self.pool.get('account.tax')
        cur_obj = self.pool.get('res.currency')
        res = {}
        if context is None:
    ...
    def _starview_amount_line(self, cr, uid, ids, field_name, arg, context=None):
        tax_obj = self.pool.get('account.tax')
        cur_obj = self.pool.get('res.currency')
        res = {}
        if context is None:
            context = {}
        for line in self.browse(cr, uid, ids, context=context):
            price = line.price_unit * (1 - (line.x_price_starview_discount or 0.0) / 100.0)
            taxes = tax_obj.compute_all(cr, uid, line.tax_id, price, line.product_uom_qty, line.product_id,
                                        line.order_id.partner_id)
            cur = line.order_id.pricelist_id.currency_id
            res[line.id] = cur_obj.round(cr, uid, cur, taxes['total'])
    ...
    def _starview_get_price_reduce(self, cr, uid, ids, field_name, arg, context=None):
        res = dict.fromkeys(ids, 0.0)
        for line in self.browse(cr, uid, ids, context=context):
            res[line.id] = line.price_starview_total / line.product_uom_qty
        return res
    ...
    _columns = {
        ...
        'invoice_lines': fields.many2many('account.invoice.line', 'sale_order_line_invoice_rel', 'order_line_id',
                                          'invoice_id', 'Invoice Lines', readonly=True, copy=False),
        'invoiced': fields.function(_fnct_line_invoiced, string='Invoiced', type='boolean',
                                    store={
                                        'account.invoice': (_order_lines_from_invoice, ['state'], 10),
                                        'sale.order.line': (
                                        lambda self, cr, uid, ids, ctx=None: ids, ['invoice_lines'], 10)
                                    }),
        'price_unit': fields.float('MSRP', required=True, digits_compute=dp.get_precision('Product Price'),
                                   readonly=True, states={'draft': [('readonly', False)]}),
        'price_subtotal': fields.function(_amount_line, string='Total', digits_compute=dp.get_precision(
            'Account')),
        'price_basetotal': fields.function(_base_amount_line, string='Total', digits_compute=dp.get_precision(
            'Account')),

        'price_reduce': fields.function(_get_price_reduce, type='float', string='U/C',
        ...
        'address_allotment_id': fields.many2one('res.partner', 'Allotment Partner',
                                                help="A partner to whom the particular product needs to be allotted."),
        ...
        'discount': fields.float('Discount (%)', digits_compute=dp.get_precision('Discount'), readonly=True,
                                 states={'draft': [('readonly', False)]}),
        'x_price_starview_discount': fields.float('Starview Discount (%)', digits_compute=dp.get_precision('Discount'),
                                              readonly=True,
                                 states={'draft': [('readonly', False)]}),
        'price_starview_uc': fields.function(_starview_get_price_reduce, type='float', string='U/C',
                                        digits_compute=dp.get_precision('Product Price')),
        'price_starview_total': fields.function(_starview_amount_line, string='Total', digits_compute=dp.get_precision(
            'Account')),
        ...
        'salesman_id': fields.related('order_id', 'user_id', type='many2one', relation='res.users', store=True,
                                      string='Salesperson'),
    ...
    def create(self, cr, uid, values, context=None):
        if values.get('order_id') and values.get('product_id') and any(
                        f not in values for f in ['name', 'price_unit', 'type', 'product_uom_qty', 'product_uom']):
            order = self.pool['sale.order'].read(cr, uid, values['order_id'],
                                                 ['pricelist_id', 'partner_id', 'date_order', 'fiscal_position'],
                                                 context=context)
            defaults = self.product_id_change(cr, uid, [], order['pricelist_id'][0], values['product_id'],
                                              ...
                                              fiscal_position=order['fiscal_position'][0] if order[
                                                  'fiscal_position'] else False,
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            if rec.state not in ['draft', 'cancel']:
                raise osv.except_osv(_('Invalid Action!'),
                                     _('Cannot delete a sales order line which is in state \'%s\'.') % (rec.state,))
...
class mail_compose_message(osv.Model):
    ...
    def send_mail(self, cr, uid, ids, context=None):
        context = context or {}
        if context.get('default_model') == 'sale.order' and context.get('default_res_id') and context.get(
                'mark_so_as_sent'):
